...
app.post('/api/generate', async (req, res) => {
  ...
  const selectedStyles = communicationStyles
    .map(s => styleNames[langKey][s] || s)
    .join(', ')
...
${dontDo ? `- Co nechci dělat / témat se chci vyhnout: ${dontDo}` : ''}
    ...
    for await (const event of stream) {
      if (event.type === 'content_block_start') {
        if (event.content_block.type === 'text') {
          ...
          const searchStatus = language === 'cs'
            ? '🔍 Prohledávám aktuální trendy v oboru...'
            : '🔍 Searching for current industry trends...'
          res.write(`data: ${JSON.stringify({ type: 'searching', message: searchStatus })}\n\n`)
        }
